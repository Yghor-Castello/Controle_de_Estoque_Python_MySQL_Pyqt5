...
def lista():
    ...
    #Leitura das Linhas e colunas (COMANDO FETCHALL)
    leitura_banco = cursor.fetchall()

    #Definindo a Contagem de Linhas da Tabela
    lista.tableWidget.setRowCount(len(leitura_banco))
    #Definindo a Contagem de Colunas da Tabela
...
def excluir():
    ...
    #Conferencia das variaveis criadas no banco de dados (produto, preço, estoque)
    conexao.commit()
...
def inserir():
    ...
    #Responsável por rodar os comandos SQL em Python (CURSOR)
    cursor = conexao.cursor()
    comando_SQL = 'INSERT INTO produtos (produto, preco, estoque) VALUES (%s, %s, %s)'
    dados = (str(produto), str(preco), estoque)
...
#Gerar Aplicação (Carregar as Aplicações)
app = QtWidgets.QApplication([])
formulario = uic.loadUi('tela_formulario.ui')
